...
img_start = 10
img_end = 50
gt_folder = "val-gt-nusc"
test_folder = "val-nusc"
save_path = "metrics"
os.makedirs(save_path, exist_ok=True)
print("Test FVD:", test_folder)
...
if os.path.exists(gtpath):
    ...
    video_tmp = []
    checkpoint, start_id = find_latest_checkpoint(save_path, gt_folder)
    ...
    # for index in tqdm.tqdm(range(2692)): # 5384
        ...
        for i in range(img_start, img_end):
            img_path = f"test_videos/{gt_folder}/sliding_{step}/{i}.png"
            retry_count = 5
                ...
                try:
                    ...
                    if attempt < retry_count - 1:
                        ...
                        if checkpoint is not None:
                            videos1 = torch.cat([checkpoint, videos1], dim=0)
                        print("Current videos1", videos1.shape)
                        torch.save(videos1, f"{save_path}/{gt_folder}-{index}.pt")
                        exit()
            # gfile = Image.open()
